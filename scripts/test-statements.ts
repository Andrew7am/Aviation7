...
console.log('\n19. An opening balance closes off the payments behind it too');
{
  // The fault this pins: an opening date gated tickets but not top-ups, so a
  // wallet re-baselined after a long payment history added that whole history
  // back on top of the new figure. NSA carried 5,150,031.50 of payments; the
  // balance would have read 5,153,827.26 against a real 3,795.76.
  const wallet = {
    id: 'w', vendorName: 'NSA', initialBalance: 3795.76,
    openingDate: '2026-09-20', currentBalance: 0, userId: 'u',
  } as any;
  const led = [
    tkt({ source: 'NSA', date: '2026-01-15', amount: 500000 }),   // settled history
    tkt({ source: 'NSA', date: '2026-09-25', amount: 1200 }),     // after the line
  ];
  const tops = [
    { id: 'p1', vendorId: 'w', vendorName: 'NSA', amount: 700000, date: '2026-01-10' },
    { id: 'p2', vendorId: 'w', vendorName: 'NSA', amount: 5000, date: '2026-09-22' },
  ] as any[];

  check('the old payment is not added back',
        Math.round(calcVendorBalance(wallet, led as any, tops) * 100) / 100, 7595.76);

  const a = ledgerAccount('NSA', [], led, tops,
    { initialBalance: 3795.76, openingDate: '2026-09-20' }, '2026-09-30');
  check('and the screen agrees with the wallet', a.balance,
        Math.round(calcVendorBalance(wallet, led as any, tops) * 100) / 100);
}

console.log('\n20. The account begins at the baseline, not at the oldest row');
{
  const led = [
    tkt({ source: 'NSA', date: '2025-06-10', amount: 900000 }),
    tkt({ source: 'NSA', date: '2026-09-25', amount: 1200 }),
  ];
  const tops = [{ id: 'p', vendorId: 'w', vendorName: 'NSA', amount: 5000, date: '2026-09-22' }] as any[];
  const a = ledgerAccount('NSA', [], led, tops,
    { initialBalance: 3795.76, openingDate: '2026-09-20' }, '2026-09-30');

  check('no period reaches behind the line',
        a.periods.every(p => p.from >= '2026-09-20'), true);
  check('the first opens at the stated figure', a.periods[0].opening, 3795.76);
  check('only the rows after it are counted',
        a.periods.reduce((n, p) => n + p.tickets.length, 0), 1);
  for (const p of a.periods)
    check(`${p.from} still foots`,
          Math.round((p.opening + p.paid - p.issued + p.refunded) * 100) / 100, p.closing);
}

console.log('\n21. A statement from behind the baseline is not argued with');
{
  const old = stm({
    vendorName: 'NSA', periodStart: '2026-06-01', periodEnd: '2026-06-30',
    openingBalance: 0, closingBalance: -23704.04, billed: 1714, paid: 0,
  });
  const led = [tkt({ source: 'NSA', date: '2026-09-25', amount: 1200 })];
  const a = ledgerAccount('NSA', [old], led, [],
    { initialBalance: 3795.76, openingDate: '2026-09-20' }, '2026-09-30');

  check('their old figure is not quoted', a.statedBalance, null);
  check('nor a difference against it',    a.balanceGap, null);
  check('and no period carries it',       a.periods.every(p => p.statement === null), true);

  // With no baseline, the same statement is still compared - the rule is the
  // baseline, not the age of the statement.
  const b = ledgerAccount('NSA', [old], led, [], { initialBalance: 3795.76 }, '2026-09-30');
  check('without a baseline it is still there', b.statedBalance, -23704.04);
}

console.log('\n22. With no opening date nothing changes');
{
  const wallet = {
    id: 'w', vendorName: 'Ibtekar', initialBalance: 0, currentBalance: 0, userId: 'u',
  } as any;
  const led = [tkt({ date: '2026-08-05', amount: 1000 })];
  const tops = [{ id: 'p', vendorId: 'w', vendorName: 'Ibtekar', amount: 2000, date: '2020-01-01' }] as any[];
  check('an ancient payment still counts',
        Math.round(calcVendorBalance(wallet, led as any, tops) * 100) / 100, 1000);
}
